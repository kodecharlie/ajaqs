...
public class SetProjectTag extends GetOrSetTag
{
    ...
    private static final Logger logger =
        Logger.getLogger(SetProjectTag.class.getName());
        ...
        throws JspException
    {
        ...
        Integer projId = (Integer)TagHelper.eval
            ("project", m_project, Integer.class, this, pageContext);
        ...
        if (null != m_emptyOk) {
            Boolean b = (Boolean)TagHelper.eval
                ("emptyOk", m_emptyOk, Boolean.class, this, pageContext);
            emptyOk = b.booleanValue();
        }

        // Get the FaqUser, which will be used to get the Project.
        ...
        try {
            ...
            if (! emptyOk) {
                throw (JspTagException)new JspTagException
                    ("Could not find project \"" + projId
                     + "\"").initCause(pnfe);
            }

            // Project not found, so force it to null
            project = null;
        }

        if (null == project) {
            if (emptyOk) {
                // We just set the variable to the empty string.
