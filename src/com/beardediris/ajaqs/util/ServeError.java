 ...
 */
public abstract class ServeError extends HttpServlet
    implements Constants
{
    private static final Logger logger =
        Logger.getLogger(ServeError.class.getName());

    /**
     * URL triggered when an exception occurs.
     */
    private final static String URL_SHOW_EXCEPTION =
        "/jsp/site/error.jsp";
        ...
        throws ServletException, IOException
    {
        ...
        if (null == rd) {
            // Hopefully this never happens.  This would really
            // defeat the purpose of implementing this method in
            // the first place.
            throw new ServletException
                ("Could not get RequestDispatcher to forward request.");
        }

        // Save the exception in the request context.  We later will
        // process the text of the exception in a custom JSP tag.
        req.setAttribute(INTERNAL_EXCEPTION, ex);
        try {
            rd.forward(req, resp);
        } finally {
            // Remove exception from request context.
