...
public final class GetAttachment extends ServeError
{
    private static final Logger logger =
        Logger.getLogger(GetAttachment.class.getName());
        ...
        throws ServletException, IOException
    {
        ...
        if (null != logon && logon.length() > 0) {
            try {
                ...
                throw (ServletException)new ServletException
                    ("FaqUser with logon \"" + req.getRemoteUser()
        ...
        if (null != parameter && parameter.length() > 0) {
            ...
            try {
                ...
                throw (ServletException)new ServletException
                    ("\"" + PROJECT + "\" parameter must be a valid integer")
                    .initCause(nfe);
            }
            try {
            ...
            throw new ServletException
                ("\"" + PROJECT + "\" parameter must be specified");
        }
        ...
        if (null != parameter && parameter.length() > 0) {
            ...
            try {
                ...
                throw (ServletException)new ServletException
                    ("\"" + FAQ + "\" parameter must be a valid integer")
                    .initCause(nfe);
            }
            try {
            ...
            throw new ServletException
                ("\"" + FAQ + "\" parameter must be specified");
        }
        ...
        if (null != parameter && parameter.length() > 0) {
            ...
            try {
                ...
                throw (ServletException)new ServletException
                    ("\"" + QUESTION + "\" parameter must be a valid integer")
                    .initCause(nfe);
            }
            try {
            ...
            throw new ServletException
                ("\"" + QUESTION + "\" parameter must be specified");
        }
        ...
        if (null != parameter && parameter.length() > 0) {
            ...
            try {
                ...
                throw (ServletException)new ServletException
                    ("\"" + ANSWER + "\" parameter must be a valid integer")
                    .initCause(nfe);
            }
            try {
            ...
            throw new ServletException
                ("\"" + ANSWER + "\" parameter must be specified");
        }
        ...
        if (null != parameter && parameter.length() > 0) {
            ...
            try {
                ...
                throw (ServletException)new ServletException
                    ("\"" + ATTACHMENT + "\" parameter must be a "
                     + "valid integer").initCause(nfe);
            }
            try {
            ...
            throw new ServletException
                ("\"" + ATTACHMENT + "\" parameter must be specified");
        }
        ...
        // Set the "Content-Disposition" to specify a default
        // filename.  FIXME 'attachment', 'inline' choices should be
        // configurable.
        ...
        // This may not be desirable in some cases, or perhaps
        // even workable.  FIXME this probably should be an option.
        resp.setHeader("Cache-Control", "private");

        // Write attachment to response output stream, 1K at a time.
        OutputStream ostream = resp.getOutputStream();
        BufferedInputStream istream =
            new BufferedInputStream(attachment.getAttachment());
        ...
        throws ServletException, IOException
    {
        ...
        // Close database here since we are not forwarded to another JSP.
        try {
            query.getDb().close();
        } catch (PersistenceException pe) {
            Exception srvEx = createEx;
            if (null == srvEx) {
                srvEx = (Exception)new ServletException
        ...
        throws ServletException, IOException
    {
        ServletException srvEx = new ServletException
            ("Cannot download attachment with POST.");
        serveError(req, resp, srvEx);
    }
}
