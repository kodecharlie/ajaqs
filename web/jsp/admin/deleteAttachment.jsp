...
<ajq:setAttachment attachment="${param.attachment}" answerid="answer"
  scope="page" var="attachment"/>

<c:set var="action">
  <c:out value="/ajaqs/servlets/admin/deleteattachment?project=${project.id}&faq=${faq.id}&question=${question.id}&answer=${answer.id}&attachment=${attachment.id}" escapeXml="false"/>
</c:set>

<c:set var="synopsis">
  <c:out value="${attachment.fileName}"/> - <c:out value="${attachment.descr}"/>
</c:set>
...
<tmplt:insert template="/jsp/admin/deleteTemplate.jsp">
  ...
  <tmplt:put name="synopsis" content="${synopsis}" direct="true" />
